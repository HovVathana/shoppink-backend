...
const optimizeDatabaseUrl = (originalUrl) => {
  ...
  if (isVercel) {
    ...
    params.set("pool_timeout", "15"); // 5 seconds pool timeout
    params.set("connect_timeout", "10"); // 3 seconds connection timeout
  ...
  console.log(
    `Database URL optimized for ${isVercel ? "Vercel" : "Railway/Local"}:`,
  );
...
const testConnectionPerformance = async (prisma) => {
  ...
  try {
    ...
    if (duration > 1000) {
      console.warn(
        `⚠️  Slow database connection detected (${duration}ms). Consider optimizing your DATABASE_URL or Railway region.`,
      );
    }
